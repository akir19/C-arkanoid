...
void autoMoveBall() {
    ...
    if ((mas[ball.iy][ball.ix]=='#') || (mas[ball.iy][ball.ix]=='@')) {
        if (mas[ball.iy][ball.ix]=='@') hitCnt++;
        if ((ball.ix!=bl.ix)&&(ball.iy!=bl.iy)) {
            if (mas[bl.iy][ball.ix]==mas[ball.iy][bl.ix]) bl.alfa=bl.alfa+M_PI;
            else {
                if (mas[bl.iy][ball.ix]=='#') bl.alfa=(2*M_PI-bl.alfa)+M_PI;
...
void init(int lvl) {
    ...
    if (lvl==2) {
        for (int i=20; i<50; i++) mas[10][i]='#';
    }
    if (lvl==3) {
        for (int i=1; i<10; i++)
            for (int j=1; j<65; j+=7) mas[i][j]='#';
    }
}
